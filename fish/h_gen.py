...
def get_heatmap_from_transformation(img, model, transformation=lambda x: x, inverse=lambda x: x, debug: object = False,
                                    last=False, inception=False):
    if debug:
        print(img.shape)
        plt.imshow(array_to_img(img), interpolation="none")
        plt.show()
...
def get_5_masks_w(metadata, mask_size=(100, 100), secu0=3, secuint=2, secu1=2, option=0):
    import math
